...
public class QualityFacadeDAO extends AbstractFacade<Quality> {
    ...
    public QualityFacadeDAO() {
     ...
     public List<Quality> getQualitiesByUserName(String name) {
        ...
        try {
            Query query = em.createNativeQuery("SELECT * FROM app_users INNER JOIN app_qualities ON app_qualities.user_id=app_users.id WHERE app_users.name='"+name+"';",User.class);
